...
const Field = ({ input,
  ...
  (
    <div className={`simple-form__control-group text-area__control-label ${(touched && error) ? 'has-error' : ''}`}>
      {label !== '' && <label htmlFor={fieldId} className="control-label">{label}</label>}
      <div className="simple-form__control-group textarea-control">
